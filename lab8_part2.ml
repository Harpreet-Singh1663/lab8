...
module MakeStack (Element: SERIALIZE) : (STACK with type element = Element.t) =
  struct
    ...
    let map (f : element -> element) (s : stack) : stack = List.map

    let filter (f : element -> bool) (s : stack) : stack = List.filter

    let fold_left (f : 'a -> element -> 'a) (init : 'a) (s : stack) : 'a = List.fold_left
